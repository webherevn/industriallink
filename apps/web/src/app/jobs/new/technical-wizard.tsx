...
export function NewJobTechnicalWizard({
  onSwitchTrack,
}: {
  ...
  const mutation = useMutation({
    mutationFn: (publish: boolean) =>
      createJob({
        ...
        jobTrack: form.jobTrack || JobTrack.Technical,
      ...
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          ...
          <p className="mt-1 text-sm text-slate-500">
            Soạn tin kỹ thuật theo 5 bước — AI hỗ trợ tiêu đề, JD và mức lương thị trường.
          </p>
          <JobTrackToggle
            value={JobTrack.Technical}
            onChange={onSwitchTrack}
            className="mt-3"
          />
        </div>
        <Button
